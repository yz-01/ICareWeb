...
class CompanyUserDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('action', function ($item) {
                return view('admin.company_users.action', compact('item'));
            })
            ->addColumn('email', function($item){
                return $item->email ?: '-';
            })
            ->addColumn('company_user_status', function($item){
                ...
                $action .= '</div>';
                $action .= '<form id="update-company-user-status-'.$item->id.'" action="'.route("admin.company_users.updateStatus", $item->id).'" class="d-none" method="post">'
                            .csrf_field().method_field("PUT").
                           '</form>';
                return $action;
            })
            ->rawColumns(['action','company_user_status', 'image']);
    }

    /**
     * Get query source of dataTable.
     *
     * @param \App\Models\Admin\CompanyUser $model
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function query(CompanyUser $model)
    {
    ...
    {
        return $this->builder()
            ->setTableId('admin-company_users-table')
            ->columns($this->getColumns())
            ->ajax([
                'url' => route('admin.company_users.index'),
                'data' => 'function(d) {
                    d.code = $("#code").val();
                    d.name = $("#name").val();
                    d.company_name = $("#company_name").val();
                    d.email = $("#email").val();
                    d.company_user_status = $("#company_user_status").val();
                }',
            ])
            ...
            ->initComplete('function() {
                    $(".datatable-input").on("change",function () {
                        $("#admin-company_users-table").DataTable().ajax.reload();
                    });
                    ...
                    $("#clearBtn").on("click",function () {
                        ...
                        $("#email").val(null);
                        $("#company_user_status").val(null);
                        $("#company_user_status").change();
                        $("#admin-company_users-table").DataTable().ajax.reload();
                    });
                    $("#admin-company_users-table").on("click", ".delFunc", function(e) {
                        var id = $(this).data("id");
                        var form = $("#delete-company-user-"+id);
                        Swal.fire({
                            title: "Are you sure?",
                    ...
                    $("#admin-company_users-table").on("change", ".change-status", function(e) {
                        var $this = $(this);
                        var id = $(this).data("id");
                        var form = $("#update-company-user-status-"+id);
    ...
    protected function getColumns()
    {
        return [
            ...
            Column::make('company_user_status')->title('Status')->orderable(false),
    ...
    {
        return 'Admin\CompanyUser_' . date('YmdHis');
    }
}
