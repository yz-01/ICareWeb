...
class AnnouncementDataTable extends DataTable
{
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('action', function ($item) {
                return view('nurse.announcements.action', compact('item'));
            })
    ...
    {
        return $this->builder()
                    ->setTableId('nurse-announcement-table')
                    ->columns($this->getColumns())
                    ->ajax([
                        'url' => route('nurse.announcements.index'),
                        'data' => 'function(d) {
                            d.title = $("#title").val();
                    ...
                    ->initComplete('function() {
                            $(".datatable-input").on("change",function () {
                                $("#nurse-announcement-table").DataTable().ajax.reload();
                            });
                            ...
                            $("#nurse-announcement-table").on("click", ".delFunc", function(e) {
                            ...
                            $("#nurse-announcement-table").on("click", ".resFunc", function(e) {
    ...
    {
        return 'Nurse\Announcement_' . date('YmdHis');
    }
}
