    ...
    */
    ...
    protected function create(Request $request)
    {
        $this->validate($request, [
            'name' => 'required|string|max:255',
            'identity_card' => 'required|string|max:255|unique:trainers',
        ...
        if($check_agent_referral_code || $check_merchant_referral_code || $check_center_referral_code || $check_trainer_referral_code || $check_company_referral_code || $check_customer_referral_code || $request->referral_code == null)
        {
            ...
            $trainer = Trainer::create([
                ...
                'own_referral_code' => 'T'.$request->phone,
            ...
            if($last_trainer){
                $add_trainer_code_number = substr($trainer->code,-4) + 1;
                $trainer->update([
                    'code' => "T".str_pad($add_trainer_code_number, 4, '0', STR_PAD_LEFT),
                ]);
            }
            else{
                $trainer->update([
                    'code' => "T".str_pad(1, 4, '0', STR_PAD_LEFT),
                ]);
            }
    ...
    public function showRegistrationForm()
    {
        ...
        return view('trainer.auth.register', compact('countries', 'security_questions'));
    }
}
