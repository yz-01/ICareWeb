<div>
    <div class="row">
        <div class="col-3">
            <a href="{{route('admin.agents.show',$item->id)}}">
        ...
        <div class="col-3">
            ...
            <form id="delete-admin-{{$item->id}}" method="POST" action="{{ route('admin.agents.destroy', $item->id) }}" class="destroy_{{$item->id}} d-none">
                {{ csrf_field() }}
                {{ method_field('DELETE') }}
            </form>
        </div>
