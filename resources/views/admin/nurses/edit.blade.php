...
<div class="row">
    <div class="col-12">
        <div class="page-title-box d-flex align-items-center justify-content-between">
            <div>
                <h4 class="mb-sm-0 font-size-18">Nurse Edit - {{ $nurse->name }}</h4>
                <ol class="breadcrumb m-0 mt-3">
                    <li class="breadcrumb-item">
                        <a href="{{ route('admin.nurses.index') }}">Nurse List</a>
                    </li>
                    <li class="breadcrumb-item active">Nurse Edit</li>
                </ol>
            </div>
        </div>
    </div>
...
<div class="row">
    <div class="col-lg-6">
        <form action="{{ route('admin.nurses.update', $nurse->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title mb-4">Nurse Information</h4>
                    <div class="form-group mb-3">
                        ...
                        <div class="file-upload" style="border-radius:65px;">
                            ...
                            @else
                                <img id="imageResult1" src="{{ asset($nurse->image) }}" class="w-100 h-100" style="object-fit:contain;">
                            @endif
                        </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('branch_id') is-invalid @enderror" name="branch_id" required>
                            ...
                            <option value="{{ $branches->id }}" {{ $nurse->branch_id == $branches->id ? 'selected' : '' }}>{{ $branches->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" placeholder="Name" value="{{ old('name') ?: $nurse->name }}" required>
                        @error('name')
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input type="tel" class="form-control @error('identity_card') is-invalid @enderror" name="identity_card" placeholder="Identity Card" value="{{ old('identity_card') ?: $nurse->identity_card }}">
                        @error('identity_card')
                        <div class="invalid-feedback">
                            <span>{{ $message }}</span>
                        </div>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input type="tel" class="form-control @error('phone') is-invalid @enderror" name="phone" placeholder="Phone" value="{{ old('phone') ?: $nurse->phone }}">
                        @error('phone')
                        <div class="invalid-feedback">
                            <span>{{ $message }}</span>
                        </div>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input type="email" class="form-control @error('email') is-invalid @enderror" name="email" placeholder="Email" value="{{ old('email') ?: $nurse->email }}" required>
                        @error('email')
                        <div class="invalid-feedback">
                            <span>{{ $message }}</span>
                        </div>
    ...
    <div class="col-lg-6">
        <div class="row">
            <div class="col-lg-12">
                <form action="{{ route('admin.nurses.updatePassword', $nurse->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="card">
                        <div class="card-body">
                            <h4 class="card-title mb-4">Nurse Account Password Change</h4>
                            <div class="card mb-2">
